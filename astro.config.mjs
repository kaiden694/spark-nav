...
function generateCreatorAvatarSvg(identifier) {
...
function viteMediaProxyPlugin() {
  return {
    name: 'vite-media-proxy-plugin',
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        ...
        if (targetUrl && (targetUrl.startsWith('http://') || targetUrl.startsWith('https://'))) {
          ...
          remote = `${upstreamBase.replace(/\/+$/, '')}/${cleanKey}`;
        }
        ...
        if (remote) {
          try {
            ...
            // Fallback to avatar svg
          }
        }

        const seedIdentifier = key || targetUrl || 'SparkNav';
        const avatarSvg = generateCreatorAvatarSvg(seedIdentifier);
        res.writeHead(200, {
          'Access-Control-Allow-Origin': '*',
...
export default defineConfig({
  site: process.env.SITE_URL || 'https://sparknav.pages.dev',
  output: 'static',
  build: {
    format: 'file',
  },
  vite: {
    plugins: [viteMediaProxyPlugin()]
  }
});
